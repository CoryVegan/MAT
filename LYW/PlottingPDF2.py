    ...
    for fNo in figNo2SubNo2Chs.keys():
        ...
        for sNo in figNo2SubNo2Chs[fNo].keys():
            ...
            if secondaryAxisChs[k]=="Iq_UUT":
                IqCHL = np.divide(ch_data[5],ch_data[1])
                plt.plot(ch_data['time'],IqCHL,'.-',label="UUT_Iq")
                plt.autoscale(enable=True, axis='x', tight=True) 
                                
            if secondaryAxisChs[k]=="Id_UUT":    
                IdCHL = np.divide(ch_data[3],ch_data[1])
                plt.plot(ch_data['time'],IdCHL,'.-',label="UUT_Id")
                plt.autoscale(enable=True, axis='x', tight=True)    
            
            if secondaryAxisChs[k]=="Iq_POC":
                PoC_QData = np.divide(ch_data[6],100)  # per unitization 
                IqCHLPoC = np.divide(PoC_QData,ch_data[1])
                plt.plot(ch_data['time'],IqCHLPoC,'.-',label="PoC_Iq")
                plt.autoscale(enable=True, axis='x', tight=True) 
                                
            if secondaryAxisChs[k]=="Id_POC":    
                PoC_PData = np.divide(ch_data[4],100)   # per unitization     
                IdCHLPoC = np.divide(PoC_PData,ch_data[1])
            ...
            ax.set_xlim(xmin=round(ch_range[0]['min']),xmax=round(ch_range[0]['max']))
...
if __name__ == '__main__':
    ...
    file_name = "C:\\UserData\\z003sarm\\Documents\\Emma Wang\\Liying_Wang_PTI\\03_Projects\\08 WSTECH\\01_PSSE Simulation\\MAT\\PlottingPDF_Gen_VPQ.csv" 
    OUTDIR = "C:\\UserData\\z003sarm\\Documents\\Emma Wang\\Liying_Wang_PTI\\03_Projects\\02_GidginbungSolarFarm\\MAT\\Test Files\\Under_Frequency_Trip_Test\\Results\\" #must end in \\ e.g. 'Results\\'
